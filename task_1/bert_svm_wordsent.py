...
train_dict = json.load(open(my_loc+'/proc_data/train_data.json', 'r'))
val_dict = json.load(open(my_loc+'/proc_data/val_data.json', 'r'))
...
for emb_type in emb_list:
    ...
    pickle.dump({'best_pca': best_pca_nk}, open(my_loc+'/models/'+fname+'_'+emb_type+'_norm%s.pkl'%(args.normalize), 'wb'))
    classifier.save_to_file(my_loc+'/models/'+fname+'_'+emb_type+'_norm%s.dt'%(args.normalize))
